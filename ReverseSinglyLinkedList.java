 ...
 *  	for writing recursive functions for the following tasks:
 ...
 *  Ver 2.0: 2018/09/09
 *  Writing recursive functions for the following tasks:
 *  (i) reverse the order of elements of the SinglyLinkedList class,
 *	(ii) print the elements of the SinglyLinkedList class, in reverse order.
...
public class ReverseSinglyLinkedList<T> implements Iterable<T> {
    ...
    // Helper function to reverse Singly Linked List
    public void reverseSinglyLinkedList(){
    	 ReverseSinglyLinkedList<T> rslt = new ReverseSinglyLinkedList<T>();
    	 rslt.head.next = reverseSLL(head.next, rslt.head.next);
    	 rslt.tail = this.head;
    	 rslt.size = this.size;
    	 rslt.printList();
    }
    ...
    public Entry<T>  
    	reverseSLL(Entry<T> node, Entry<T> prevNode) {
    	if (node.next == null) {
    		head.next = node;
    		node.next = prevNode;
    		return head.next;
    	}
    	Entry<T> temp = node.next;
		node.next = prevNode;
		reverseSLL(temp,node);
		return head.next;
    }
    ...
    public static void main(String[] args) throws NoSuchElementException {
        ...
        ReverseSinglyLinkedList<Integer> lst = new ReverseSinglyLinkedList<>();
        for(int i=1; i<=n; i++) {
